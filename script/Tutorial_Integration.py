...
hyperparameter_defaults = dict(
    seed=42,
    dataset_name="covid_subsampled",  # Dataset name
    do_train=True,  # Flag to indicate whether to do update model parameters during training
    load_model="/mnt/Data1/frx/scGPT_Rank/pretrained_model",  # Path to pre-trained model
    ...
    # default=False, help="Flag to stop memory allocation tracing during training. This could speed up training on Windows."
)
run = wandb.init(
...
save_dir = Path(f"./save/dev_{dataset_name}_{prompt_type.replace('-', '_').upper()}/")
...
elif dataset_name == "Lung_atlas_public":
    adata = sc.read(data_dir / f"{dataset_name}.h5ad")

    # Restore to raw count
    adata.X = np.expm1(adata.X)
    adata.X = adata.X.multiply(1 / adata.obs['size_factors'][:, None])

    ori_batch_col = "batch"
    adata.obs["celltype"] = adata.obs["cell_type"].astype("category")
    data_is_raw = True

elif dataset_name == "pbmc_2_batch_base_balanced":
    adata = sc.read(data_dir / f"{dataset_name}.h5ad")
    ori_batch_col = "batch"
...
logger.info("-" * 89)
for k, v in not_in_pretrained_dict.items():
    logger.info(f"Model params {k} not in pretrained weights with shape {v.shape}")

pre_freeze_param_count = sum(dict((p.data_ptr(), p.numel()) for p in model.parameters()).values())

for name, para in model.named_parameters():
    para.requires_grad = False
for param in model.batch_encoder.parameters():  # batch encoder not in pretrained weights
    param.requires_grad = True
for param in model.grad_reverse_discriminator.parameters():  # grad_reverse_discriminator not in pretrained weights
    param.requires_grad = True
...
def eval_testdata(
    ...
    if "cls" in include_types:
        ...
        fig = sc.pl.umap(
            ...
            # return_fig=True,
            show=False,
            save=f"_{dataset_name}_{prompt_type}_embeddings_batch_umap[cls]_e{best_model_epoch}"
        )
        ...
        fig = sc.pl.umap(
            ...
            # return_fig=True,
            show=False,
            save=f"_{dataset_name}_{prompt_type}_embeddings_celltype_umap[cls]_e{best_model_epoch}"
        )
...
for epoch in range(1, config.epochs + 1):
    ...
    if epoch % config.save_eval_interval == 0 or epoch == config.epochs:
        ...
        #     save_dir / f"embeddings_batch_umap[cls]_e{best_model_epoch}.pdf", dpi=800
        # )
        #
        # results["celltype_umap"].savefig(
        #     save_dir / f"embeddings_celltype_umap[cls]_e{best_model_epoch}.pdf", dpi=800
        # )
