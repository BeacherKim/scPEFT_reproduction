...
amp = True
load_model = "/mnt/Data3/22frx/scGPT_pretrain/scGPT_human"
load_param_prefixs = [
    "encoder",
    "value_encoder",
    "transformer_encoder",
...
use_prompt = True
# prompt_type = 'encoder-prompt and head-prompt'  # encoder-prompt、prefix-prompt、head-prompt、condition-prompt、finetune、LoRA
...
save_dir = Path(f"./save/dev_perturb_{data_name}_{prompt_type}/")
...
decoder_param_count = sum(
    dict((p.data_ptr(), p.numel()) for n, p in model.named_parameters() if "decoder" in n).values())
...
logger.info("Total Decoder Params: %.2fM" % (decoder_param_count / 1e6,))
